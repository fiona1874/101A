...
Write your answer here: 2^(4-1)-1=7
...
answer: 0 0000 001 0.001*2^(1-7)=2^(-3)*2^(-6)=1/512

e. __What is the largest finite value that can be expressed with this sytem?__

answer: 0 1110 111 1.111*2^(8+4+2-7)=240
...
* Do part (b) using x0 = 2 
```{r}
fb <- function(x) exp(exp(-x))
fixedpoint_show(fb, 2, iter=10)
```

* Do part (c) using x0 = 2 
```{r}
fc <- function(x) x-log(x)+exp(-x)
fixedpoint_show(fc, 2, iter=10)
```

* Do part (d) using x0 = 2, no more than 6 iterations
```{r}
fd <- function(x) x+log(x)-exp(-x)
fixedpoint_show(fd, 2, iter=6)
```
...
newtonraphson_show(f_c, 0, iter = 8)
```
...
newtonraphson_show(d, 1.1, iter = 8)
```
...
* x1 The x-value at iteration 1. The secant uses the last two values, and draws a secant line that connects those until that secant line intersects the y-axis.
...
secant_show <- function(ftn, x0, x1, iter = 5) {
  ...
  xn <- x0
  xn1 <- x1
  cat("Starting value is:", x0," and ", x1, "\n")
  ...
  for(i in 1:iter){
    ...
    fxn <- ftn(xn)
    fxn1 <- ftn(xn1)
    xnew <- xn1 - fxn1 * (xn1-xn)/(fxn1-fxn)
    cat("Next x value:", xnew, "\n")
    
    l <- min(xn, xn1, xnew)
    r <- max(xn, xn1, xnew)
    fl <- c(0,fxn, fxn1)[which(l==c(xnew,xn,xn1))]
    fr <- c(0,fxn,fxn1)[which(r==c(xnew,xn,xn1))]
    # the line segments. You will need to replace the NAs with the appropriate values
    df_points[2*i-1,] <- c(l, fl, r, fr) # vertical segment 
    df_points[2*i,] <- c(xnew, 0, xnew, ftn(xnew))   # tangent segment 
...
ggplot(df, aes(x=x,y=y,z=z)) + geom_contour(binwidth = 0.9)
...
for (i in 1:20) {
    cat(x_i, y_i, "\n")
    gg <- function(x){
        5*x^2-6*x*y_i+5*y_i^2
    }
    x_1 <- golden(gg, -1.5, 1.5)
    segments(x_i, y_i, x_1, y_i, col = "red")
    if(abs(x_1-x_i)<1e-5) break
    x_i <- x_1
    ff <- function(y){
      5*x_i^2-6*x_i*y+5*y^2
    }
    y_1 <- golden(ff, -1.5, 1.5)
    segments(x_i,y_i,x_i,y_1, col = "red")
    if(abs(y_1-y_i)<1e-5) break
    y_i <- y_1
