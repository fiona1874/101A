...
draw_chance <- function(player){
  ...
  if(card == 4){
    ...
    }else{player$go_2_space_n(29)}
  }
  if(card == 5){
    if (player$pos > 36){
      player$go_2_space_n(railroads[1])
    }else{
      nearest <- min(which(railroads > player$pos))
      player$go_2_space_n(railroads[nearest])
    }
  }
  if(card == 6){
...
taketurn <- function(player, tracking){
  ...
  # roll <- dice(verbose = TRUE)  # this will only work if you are not using the manual dice
      ...
      # if the player landed on chance, community chest
      if(player$pos %in% chance){
        draw_chance(player)
        tracking$increase_count(player$pos)}
      if(player$pos %in% community){
        draw_community(player)
        tracking$increase_count(player$pos)}
      tracking$increase_count(player$pos)
      }
...
# d. roll again for rolling doubles ---------------------------------------
      ...
      player$move_n(roll2$movement)
      if(player$pos %in% chance){draw_chance(player)}
      if(player$pos %in% community){draw_community(player)}
      tracking$increase_count(player$pos)
      #third roll
      roll3 <- dice()
      if(!roll3$doubles){
...
for(i in 1:1000){ # simulate 1000 games
  cat("#### NEW GAME",i,"##### \n")
  player1 <- player$new(pos = 1, verbose = FALSE, jail=0, in_jail=FALSE)  # new players for each game
...
space_tracking <- tracking$new(tally = rep(0,40))
player1 <- player$new(pos = 1, verbose = TRUE, jail=0)  # new players for each game
